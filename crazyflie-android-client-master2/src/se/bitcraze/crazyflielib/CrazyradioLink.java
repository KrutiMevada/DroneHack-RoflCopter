...
public class CrazyradioLink extends AbstractLink {
	 ...
	 */
	public CrazyradioLink(Context context, ConnectionData connectionData)
			throws IOException {
		this.mUsbManager = (UsbManager) context
	 ...
	 */
	private static UsbDevice searchForCrazyradio(Context context,
	 ...
	 */
	public static ConnectionData[] scanChannels(Context context)
			throws IllegalStateException {
	 ...
	 * 
	 * @param connection
	 *            connection to the USB dongle.
	 * @return array containing the found channels and datarates.
	 * @throws IllegalStateException
	 *             if the Crazyradio is not attached (the connection is
	 *             <code>null</code>).
	 */
	protected static ConnectionData[] scanChannels(
		...
		if (connection != null) {
			...
			for (int b = 0; b < 3; b++) {
				// set data rate
				connection.controlTransfer(0x40, REQUEST_SET_DATA_RATE, b, 0,
						null, 0, 100);
				connection.controlTransfer(0x40, REQUEST_START_SCAN_CHANNELS,
						0, 125, packet, packet.length, 1000);
				final int nfound = connection.controlTransfer(0xc0,
						REQUEST_GET_SCAN_CHANNELS, 0, 0, rdata, rdata.length,
						1000);
				for (int i = 0; i < nfound; i++) {
					result.add(new ConnectionData(rdata[i], b));
					Log.d(LOG_TAG, "Channel found: " + rdata[i]
							+ " Data rate: " + b);
				}
			}
		} else {
	...
	public void setContinuousCarrier(boolean continuous) {
		sendControlTransfer(0x40, REQUEST_SET_CONT_CARRIER,
				(continuous ? 1 : 0), 0, null);
	}
	...
	private final Runnable radioControlRunnable = new Runnable() {
		...
		public void run() {
			...
			while (mConnection != null) {
				try {
					CrtpPacket p = mSendQueue.pollFirst(5,
					...
					final int receivedByteCount = sendBulkTransfer(sendData,
							receiveData);
					...
					if (receivedByteCount >= 1) {
						...
						Log.w(LOG_TAG,
								"CrazyradioLink comm error - didn't receive answer");
					}
	...
	public int sendControlTransfer(int requestType, int request, int value,
			int index, byte[] data) {
		if (mConnection != null) {
			int dataLength = (data == null) ? 0 : data.length;
			return mConnection.controlTransfer(requestType, request, value,
					index, data, dataLength, TRANSFER_TIMEOUT);
		}
		return -1;
	}
	...
	public int sendBulkTransfer(byte[] data, byte[] receiveData) {
		int returnCode = -1;
		if (mConnection != null) {
			mConnection.bulkTransfer(mEpOut, data, data.length,
					TRANSFER_TIMEOUT);
			returnCode = mConnection.bulkTransfer(mEpIn, receiveData,
					receiveData.length, TRANSFER_TIMEOUT);
		}
		return returnCode;
	}
