...
/**
 * The GyroscopeController extends the TouchController and uses the gyroscope
 * sensors of the device to control the roll and pitch values. The yaw and
 * thrust values are still controlled by the touch controls according to the
 * chosen "mode" setting.
 * 
 */
public class GyroscopeThrustYawController extends TouchController {
	...
	public GyroscopeThrustYawController(Controls controls, MainActivity activity,
			DualJoystickView dualJoystickView, SensorManager sensorManager) {
	...
	public void enable() {
		super.enable();
		mSensorManager.registerListener(mSeListener, mSensor,
	...
	class AccelerometerListener implements SensorEventListener {
		...
		public void onSensorChanged(SensorEvent event) {
			float d = (float) Math.abs(Math.sqrt(event.values[0]
	...
	public float getRoll() {
		...
		roll = (float) Math.min(1.0,
				Math.max(-1, roll + mControls.getRollTrim()));

		return (roll + mControls.getRollTrim())
				* mControls.getRollPitchFactor() * mControls.getDeadzone(roll);
	}
	...
	public float getPitch() {
		...
		// Filter the overshoot
		pitch = (float) Math.min(1.0,
				Math.max(-1, pitch + mControls.getPitchTrim()));

		return (pitch + mControls.getPitchTrim())
				* mControls.getRollPitchFactor() * mControls.getDeadzone(pitch);
	}
